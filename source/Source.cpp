...
bool PlaceQueen(int column) {
	if (column == NUMBER_COLS) { // Base Condition
		...
		for (int row = 0; row < RowPlacement.size(); ++row) {
			...
			if ( RowPlacement[row] != UNASSIGN_VALUE ) // if value in row spot on board is not unassigned then continue with next iteration of loop
				continue;
			...
			RowPlacement[row] = column;// assign the column val as to which coloumn queen is in
